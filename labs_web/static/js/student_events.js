...
$(document).ready(function () {
   $.get('/student/collect-events/',function (events) {
       ...
       let reports_checked_template = '<div class="alert alert-success" role="alert">Lab {number} ({shortened}) - got {mark}<br><small>{checked}</small></div>';
       for (let i in events){
           if (events[i].category === "report-checked"){
               let event = events[i];
                event_block.innerHTML = event_block.innerHTML + reports_checked_template.
                    ...
                    replace('{mark}', event.mark).replace('{checked}', event.checked.replace(" GMT", ""))
           }
       }
   })
});